...
struct ScrollRangeHold {
    held: bool,
    settled_range: Option<Size<Pixels>>,
}
...
impl Editor {
    ...
    fn frozen_scroll_range(
        ...
        if !hold.held && !is_rewrapping {
            hold.settled_range = Some(current_range);
            return None;
        }
        let settled_range = hold.settled_range?;
        let held_range = size(
            settled_range.width.max(current_range.width),
            settled_range.height.max(current_range.height),
        );
        hold.settled_range = Some(held_range);
        Some(held_range)
    }
