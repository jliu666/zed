...
impl ProjectSearchView {
    ...
    fn replace_next(&mut self, _: &ReplaceNext, window: &mut Window, cx: &mut Context<Self>) {
        if self.entity.read(cx).pending_search.is_some() {
            self.pending_replace_next = self.entity.read(cx).phase.is_confirmed();
            return;
        }
    ...
    fn replace_all(&mut self, _: &ReplaceAll, window: &mut Window, cx: &mut Context<Self>) {
        if self.entity.read(cx).pending_search.is_some() {
            self.pending_replace_all = self.entity.read(cx).phase.is_confirmed();
            return;
        }
        let query_text = self.query_editor.read(cx).text(cx);
        let query_is_stale =
    ...
    fn entity_changed(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let preserve_view = phase.is_typing() || results_stale;
        ...
        if match_ranges.is_empty() {
            ...
            if !preserve_view {
                ...
                if is_new_search {
                    ...
                    if phase.is_confirmed() && self.query_editor.focus_handle(cx).is_focused(window)
                    {
                        self.focus_results_editor(window, cx);
                    }
                } else if !search_pending && phase.is_confirmed() {
                    self.results_editor
...
pub mod tests {
    ...
    async fn test_incremental_search_clears_stale_excerpts_on_query_change(
        ...
        assert_eq!(match_count(&search, cx), 4);
        ...
        assert_eq!(match_texts(&search, cx), vec!["delta_unique_word"]);
    }
    ...
    #[gpui::test]
    async fn test_search_on_type_resets_scroll_after_no_hit_query(cx: &mut TestAppContext) {
        init_test(cx);
        enable_search_on_type(cx);
    ...
    #[gpui::test]
    async fn test_erasing_invalid_query_clears_its_error(cx: &mut TestAppContext) {
        init_test(cx);
        enable_search_on_type(cx);
            ...
            .update(cx, |search_view, _, _| {
                assert!(
                    search_view
                        .panels_with_errors
                        .contains_key(&InputPanel::Query),
                    "an invalid regex typed with search-on-type must surface an error"
                );
            })
            .unwrap();
    ...
    #[gpui::test]
    async fn test_no_results_verdict_holds_through_researching(cx: &mut TestAppContext) {
        init_test(cx);
    ...
    async fn test_search_on_type_confirm_after_filter_change_researches(cx: &mut TestAppContext) {
        ...
        let search_view = active_search_view(&workspace, cx);
    ...
    #[gpui::test]
    async fn test_no_results_verdict_drops_while_typing_a_different_query(cx: &mut TestAppContext) {
        init_test(cx);
    ...
    async fn test_focusing_results_records_history(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            read_query_history(cx),
            vec!["ONE".to_string()],
            "engaging with the results of an unconfirmed search must record it in the history"
        );
    ...
    #[gpui::test]
    async fn test_focusing_results_during_pending_search_records_history(cx: &mut TestAppContext) {
        init_test(cx);
        enable_search_on_type(cx);
        ...
        cx.read(|cx| {
            ...
            assert!(model.phase.is_confirmed());
        });
    }
